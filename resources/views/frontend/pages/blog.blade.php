...
@section('main')
        ...
        <div class="post-area blog-area pt-110 pb-95 blog-column">
                                ...
                                <div class="single-blog">
                                    ...
                                    <h4><a href="blog-details.html">{{$value->name}}</a></h4>
                                       ...
                                       - <span class="post-date"> {{$value->created_at}}</span>
                                        ...
                                        <p>{{$value->meta_desc}}</p>
                                    <a href="{{route('blog_detail',['slug'=>$value->slug,'id'=>$value->id])}}" class="default-btn">Read more</a>
                                    </div>
                                </div>
                            </div>
                            @endforeach
            ...
            <div class="pagination-wrapper">
                <p>Showing {{ $blog->firstItem() }} to {{ $blog->lastItem() }} of {{ $blog->total() }} ({{ $blog->lastPage() }} Pages)</p>
                <nav aria-label="navigation">
                    <ul class="pagination">
                        @if ($blog->currentPage()>1)
                        <li class="page-item"><a class="page-link" href="{{ $blog->previousPageUrl() }}"><</a></li>
                        @endif
                        @for ($i = 1; $i <= $blog->lastPage(); $i++)

                        <li class="page-item {{(($blog->currentPage() == $i)? 'active' :'')}}"><a class="page-link" href="{{ $blog->url($i) }}">{{ $i }}</a></li>
                        @endfor
                        @if ($blog->currentPage()<$blog->lastPage())
                        <li class="page-item"><a class="page-link" href="{{ $blog->nextPageUrl() }}">></a></li>
                        <li class="page-item"><a class="page-link" href="{{ $blog->url($blog->lastPage())}}">>|</a></li>
                        @endif
                    </ul>
                </nav>
            </div>
